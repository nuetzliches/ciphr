...
### Fixed — every published artefact carries the notices it owes

**Two OCI labels were the whole of it.** `org.opencontainers.image.licenses=MIT OR Apache-2.0` on the
server and wrapper images, the same on the viewer's, and not one line of anybody's notice inside any
artefact. A label names a licence to a registry; it does not hand the person who pulled the image the
text the licence says they must receive. Almost every dependency here is under MIT, BSD-2-Clause,
BSD-3-Clause or ISC, and each of those requires its copyright notice to accompany a *copy* of the
software — which a compiled binary is, and so is an image containing one, and so is a bundle served to
a browser.

`deny.toml` has always decided which licences may *enter* the tree. Nothing decided whether their
conditions were met on the way out, and the answer was that they were not.

- **`THIRD-PARTY-LICENSES.md`**, generated by `ci/generate-attribution.sh`: the notice of each of the
  131 third-party crates linked into a published binary, as 114 distinct texts. The crate set is the
  union over the four triples that actually get an artefact — `x86_64`/`aarch64` `linux-gnu` for the
  server image, `linux-musl` for `ciphr-run` and `ciphr-ci` — following normal and build edges only.
  Dev-dependencies are absent because no artefact contains them, and `x86_64-pc-windows-msvc` is
  absent although `deny.toml` covers it, because no Windows artefact is published.
- **`ui/THIRD-PARTY-LICENSES.md`**, generated by `ci/generate-ui-attribution.sh`: the 23 packages in
  the viewer's runtime closure, as 12 distinct texts. The closure rather than the tree-shaken bundle,
  which contains less: reading Vite's module graph to find out exactly which modules survived would
  risk being wrong in the direction of a missing notice, so this over-attributes instead.
- **The texts are deduplicated.** Most crates ship the same Apache-2.0 file. One copy per crate would
  have produced some 27,000 lines that nobody would read, and a notice nobody reads is the failure
  this fixes rather than a stricter form of compliance.
- **A crate that ships no licence text of its own fails the generator.** All 131 carry one today, so
  the fallback that would otherwise be needed — a canonical text per identifier, chosen on the
  crate's behalf — does not exist and was not written speculatively. Deciding which text stands in
  for a missing one is a reading of somebody else's licence and belongs in a commit.
- **Both files ship where the artefact is.** `/usr/share/licenses/ciphr/` in the server image,
  `/usr/share/licenses/ciphr-ui/` in the viewer's, `/licenses/` in the wrapper's — and attached to
  every release beside the binaries, because `ciphr-run` and `ciphr-ci` are also published as bare
  executables and a bare executable carries nothing. In the viewer's image the notices sit outside
  `/usr/share/nginx/html`: `location /` ends in `try_files $uri $uri/ =404`, so anything in the
  document root becomes a public URL, and a notice is something the image carries rather than
  something the viewer serves.
- **`Dockerfile.run` no longer claims its filesystem is one file.** It is one binary and three text
  files under `/licenses`. The property that matters is unchanged — no shell, no libc, no package
  manager, so a container built from it can execute nothing but the wrapper — and the header now says
  the true version of it rather than the tidier one.

`ci/check-attribution.sh` and `ci/check-ui-attribution.sh` are new and blocking. Both are deliberately
cheap: they compare the crate set in the committed file against what a lockfile says ships, so neither
needs unpacked sources or `node_modules` and both run anywhere. What that leaves uncovered is a crate
changing its licence text within one published version, which crates.io does not permit — a version is
immutable once published, and the version is in the table. `ci/shipped-crates.sh` answers "what
ships" for the generator and the gate alike, so the two cannot drift apart on the question that
matters.

The viewer's copies of `LICENSE-MIT` and `LICENSE-APACHE` are checked byte-for-byte against the
originals. That image's build context is `ui/` and a `COPY` cannot reach outside it, so the package
carries its own copies — and a copy is a thing that drifts, most likely on the copyright line, where a
stale notice is worse than an absent one because it looks authoritative.

### Changed — the copyright holder is named as the legal entity

`LICENSE-MIT` said `nuetzliches`, which is the GitHub organisation rather than the entity that holds
the copyright. It now says `nuetzliche.IT`, and `README.md` states it too. A disclaimer of warranty
and of liability runs in favour of whoever the licence names; naming the account instead of the
holder is the kind of imprecision that costs nothing until it is the only thing being read. Every
other `nuetzliches` in the tree is a URL to the organisation and stays as it is.
