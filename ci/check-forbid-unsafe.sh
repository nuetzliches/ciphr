...
#
# `fuzz/` is out of scope, and deliberately so: libFuzzer targets are generated by
# a macro that emits an `extern "C"` entry point, so the attribute cannot be applied
# there. Nothing in `fuzz/` ships, and it is built only by the fuzz job.
set -eu
