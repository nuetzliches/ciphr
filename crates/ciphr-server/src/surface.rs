...
pub const ENTRIES: &[Entry] = &[
    ...
    Entry {
        name: "bulk_export",
        kind: Kind::Runtime,
        cost: "One request per path instead of one for all of them, at container \
               start, and since ADR-25 that is the whole cost. `ciphr-sdk`'s \
               `read_all` reads through this route where it exists and falls back to \
               ...
               code 125; that stopped being true on 2026-08-24. The trail does not \
               notice either way: this route writes one entry per secret served, \
               never one per call. It does not decide whether this deployment has \
               fetched prefixes for bait to stay out of (ADR-15): covering a prefix \
               is a property of the code that fetches, and `GET /v1/list/{prefix}` is \
               not an entry.",
        compiled_in: true,
    },
    Entry {
