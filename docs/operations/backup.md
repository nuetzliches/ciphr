...
| `ciphr backup` into a directory the service uid cannot write | `the backup could not be written to /out/store-pre.sqlite: … the destination is the thing to check, not the store: /out has to exist and be writable by the uid running this command` | **Created by following the fix for the row above.** Running as the service uid is the advice; the operator's own directory is usually not writable by it. One `chown` on the destination directory, or write into the staging directory the backup job already owns. The message names the directory because the earlier wording named only the file, and every other sentence at that moment is about the *source* |
